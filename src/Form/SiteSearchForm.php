...
class SiteSearchForm extends SearchForm
{
    ...
    public function getResults($pageLength = null, $data = null)
    {
        ...
        if (class_exists('Translatable') && singleton(SiteTree::class)->hasExtension('Translatable') && isset($data['locale'])) {
        ...
        if (strpos($keywords, '"') !== false || strpos($keywords, '+') !== false || strpos($keywords, '-') !== false || strpos($keywords,
                '*') !== false) {
            $results = DB::getConn()->searchEngine($this->classesToSearch, $keywords, $start, $pageLength,
                "\"Relevance\" DESC", $siteFilter, true, $fileFilter);
        } else {
            $results = DB::getConn()->searchEngine($this->classesToSearch, $keywords, $start, $pageLength, '',
                $siteFilter, false, $fileFilter);
        }
